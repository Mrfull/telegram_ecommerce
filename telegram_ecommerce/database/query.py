...
def search_promotions():
    command = """SELECT * FROM promotion WHERE active = 1 ORDER BY create_at"""
    promotions_that_match = (db.execute_a_query(command))
    return promotions_that_match


def search_favorites(customer_id):
    command = """SELECT * FROM favorite WHERE customer_id = %s"""
    favorites_that_match = (db.execute_a_query(command, (customer_id,)))
    return favorites_that_match
